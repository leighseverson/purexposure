...
#'   data frame.
#' @param chemicals A character string indicating active ingredients for which you
...
#'          exposure will be calculated based on townships. - (take a look @
#'          returned plotting data to make sure you like what you see before
#'          moving on to more complicated visualization functions.)}
#' }
#'
#' @examples
#' \dontrun{
...
calculate_exposure <- function(clean_pur_df, location, radius,
  ...
  which_mtrs <- df %>% dplyr::filter(long >= range$min_long &
                                       long <= range$max_long &
                                       lat >= range$min_lat &
                                       lat <= range$max_lat)
  ...
  if ("section" %in% colnames(clean_pur_df)) {
    ...
    mtrs_int <- as.character(unique(which_mtrs$MTRS))
    ...
    mtrs_int <- as.character(unique(which_mtrs$MTR))
...
calculate_exposure_bydate <- function(start_date, end_date) {
  ...
  nested_df <- list(row_out = row_out, meta_data = exp)
