...
fresno$map
```
```{r echo = FALSE, out.width = "400pt", out.height = "300pt", fig.cap = "Applied pesticides in Fresno, California from 1995 through 1998."}
knitr::include_graphics("figures/fresno_example.png")
```
...
```{r echo = FALSE, out.width = "250pt", out.height = "400pt", fig.cap = "Base Lines and Meridian lines in California."}
...
This often results in identical search results for multiple years. However, occationally the chemicals present in PUR datasets differs from year to year. For example, the active ingredient imazosulfuron was not applied in 2006, but was applied in 2007: 
...
```
```{r echo = FALSE}
load("data/plot_sun_empire.RData")
```
```{r}
...
```
```{r echo = FALSE, fig.align = "center"}
knitr::include_graphics("figures/ca_plot.png")
```
